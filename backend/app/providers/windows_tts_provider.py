...
param([string]$TextPath, [string]$LangCode, [string]$Gender, [string]$OutPath)
...
# Prefer a voice matching both language and gender; fall back to
# language-only if this Windows install doesn't have that language's
# voice in the requested gender (common — most languages beyond English
# only ship one gender's voice unless the user installs more).
$voice = $installed | Where-Object {
    $_.VoiceInfo.Culture.TwoLetterISOLanguageName -eq $LangCode -and $_.VoiceInfo.Gender -eq $Gender
} | Select-Object -First 1
if (-not $voice) {
    $voice = $installed | Where-Object {
        $_.VoiceInfo.Culture.TwoLetterISOLanguageName -eq $LangCode
    } | Select-Object -First 1
}
...
class WindowsTTSProvider(TTSProvider):
    def __init__(self, language_code: str = "en", gender: str = "female") -> None:
